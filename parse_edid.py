...
raw = bytes.fromhex("".join(ch for ch in open(sys.argv[1]).read() if ch in "0123456789abcdefABCDEF"))
print(f"EDID {len(raw)} bytes = {len(raw) // 128} blocks\n")
...
print(f"  manufacturer {chr((mfg >> 10 & 31) + 64)}{chr((mfg >> 5 & 31) + 64)}{chr((mfg & 31) + 64)}"
      ...
      f"  EDID {b0[18]}.{b0[19]}  extension blocks {b0[126]}")
vid = b0[20]
print(f"  input: {'digital' if vid & 0x80 else 'analog'} bitdepth={(vid >> 4) & 7} interface={vid & 0xF}"
      f" (5=DisplayPort, 2=HDMIa)")
for i in range(4):
    o = 54 + i * 18
    d = b0[o:o + 18]
    if d[0] or d[1]:
...
for bi in range(1, len(raw) // 128):
    ...
    if tag == 0x02:
        ...
        while p < dtdoff:
            ...
            print(f"  data block t={t}({names.get(t, '?')}) len={ln}{extra}  {body.hex()}")
            p += 1 + ln
        p, i = dtdoff, 0
        ...
        print(f"  DisplayID version 0x{ver:02x}  section size {sz}  product type {prod}")
        p, end = 5, 5 + sz
        while p < end and p < 127:
            dtag, drev, dlen = blk[p], blk[p + 1], blk[p + 2]
            body = blk[p + 3:p + 3 + dlen]
            tags = {0x03: "Type I timing", 0x22: "Type VII timing (detailed)", 0x23: "Type VIII enumerated",
                    0x24: "Type IX formula", 0x2a: "Type X formula", 0x20: "Product ID",
                    0x21: "Display params", 0x26: "Interface features", 0x32: "Vendor"}
            print(f"  data block tag=0x{dtag:02x} ({tags.get(dtag, '?')}) rev=0x{drev:02x} len={dlen}")
